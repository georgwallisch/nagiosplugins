...
#
# $Id: check_raspi_status.pl / Version 0.1 / 2018-01-24 / gw@phpco.de
#
...
# CHANGELOG:
#
# 0.1 - xx.xx.2018 - First testing version
#
...
my $vcgencmd = "/opt/vc/bin/vcgencmd";

# Constructor of core monitoring object
my $np = Monitoring::Plugin->new(
	usage => "Usage: %s -v <value> [-t <timeout>] [ -c|--critical=<threshold> ] [ -w|--warning=<threshold> ]",
	version => '0.1',
	blurb => 'This plugin checks a rasperry pi for its internal status values',
	);

# add options
$np->add_arg(
	spec => 'value|v=s',
	help => "Value to check. These are all the supplied arguments for vcgencmd.",
	);
$np->add_arg(
	spec => 'warning|w=s',
